...
vector<uint8_t> **TgaImage::readTGAPixels(ifstream *myFile, tgaHeader *header) {
    ...
    for(i=0; i < header -> height; i++)
        imData[i] = new vector<uint8_t>[header -> width];

    uint16_t h=0, w=0;
    unsigned long long j;
    for(j=0; j < imSize; j += bytesPerPoint) {

        vector<uint8_t> pixel;

        uint8_t offset;
        for(offset=0; offset < bytesPerPoint; offset++) {         // Depending on either 24- or 32-Bits per pixel,
            pixel.push_back(imRawData[j + offset]);               // 3 or 4 bytes are read into the vector, which
